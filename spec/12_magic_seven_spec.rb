...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'when the previous step is 10 and the random_number is 3' do
  ...
  describe '#play' do
    ...
    context 'when the random number is 68' do
      subject(:game_sixty_eight) { described_class.new(68) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = game_sixty_eight.play
        expect(result).to eq(7)
      end
    end

    context 'when the random number is 8' do
      subject(:game_eight) { described_class.new(8) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = game_eight.play
        expect(result).to eq(7)
      end
    end
  end
